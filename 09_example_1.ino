...
float median_from_buffer() {
  int n=buf_count>0 ? buf_count:1;
  float tmp[N];
  ...
  else return 0.5f * (tmp[n/2-1]+tmp[n/2]);
}
...
void setup() {
  ...
  for (int i=0; i<N; i++) buf[i]=_DIST_MAX;
  buf_count=0;
}
...
void loop() {
  ...
  if (dist_raw>0.0f) {
    buf[buf_idx]=dist_raw;
    buf_idx=(buf_idx+1)%N;
    if (buf_count<N) buf_count++;
  }
  float dist_median=median_from_buffer();

  if (!ema_initialized) {
    dist_ema=(dist_raw>0.0f) ? dist_raw : dist_median;
    ema_initialized=true;
  } else {
    float x=(dist_raw>0.0f) ? dist_raw :dist_ema;
    dist_ema=_EMA_ALPHA*x+(1.0f-_EMA_ALPHA)*dist_ema;
  }
